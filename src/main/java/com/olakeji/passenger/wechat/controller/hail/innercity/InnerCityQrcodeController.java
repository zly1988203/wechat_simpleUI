 ...
 *
 */
@Controller("HailInnerCityQrcodeController")
public class InnerCityQrcodeController extends HailController {
	...
	@RequestMapping("/hail/bus/toInnerCityQrcodePay")
	public String toInnerCityQrcodePay(String orderNo, HttpServletRequest request, Model model) {
		...
		LOGGER.info("中交城际网约车二维码支付订单详情数据返回 = {}, payCenter = {}, wechatUrl = {}", GsonUtil.GsonString(payInfo), payCenterUrl, wechatURL);
		...
		if (!result.getCode().equals(Constant.SUCCESS)) {
			if (result.getCode() == Constant.QRCODE_PAY_INVALID) {
				...
				return "hail/order/qrcodePayFail";
			}
			return "Error500";
		}
		Map<String, Object> resultMap = GsonUtil.GsonToMaps(GsonUtil.GsonString(result.getData()));
		...
		if (resultMap.get("settleMode").toString().equals("0")) {
			/*returnUrl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + zhongjiaoSubAppId
					...
					+ "/hail/order/toInnerCityQrcodeWXPay&connect_redirect=1&response_type=code&scope=snsapi_base&state="
					+ orderNo + "_" + providerId + "#wechat_redirect";*/
			returnUrl = this.commonService.authApiConfig().baseAuthUri(zhongjiaoSubAppId, payCenterUrl + "/hail/order/toInnerCityQrcodeWXPay",
					orderNo + "_" + providerId);
		} else {
			/*returnUrl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + appid + "&redirect_uri="
					+ wechatURL
					+ "/hail/bus/toInnerCityQrcodeWXPay&connect_redirect=1&response_type=code&scope=snsapi_base&state="
					+ orderNo + "_" + providerId + "#wechat_redirect";*/
			returnUrl = this.commonService.authApiConfig().baseAuthUri(appid, wechatURL + "/hail/bus/toInnerCityQrcodeWXPay",
					orderNo + "_" + providerId);
		}
		return "redirect:" + returnUrl;
	}

	@RequestMapping("/hail/bus/toInnerCityQrcodeWXPay")
...
//						return "redirect:" + "/bus/toHailInnerCityQrcodeOrderPay?uuid=" + uuid + "&orderNo=" + orderNo;
						String urlStr = "/bus/toHailInnerCityQrcodeOrderPay?uuid=" + uuid + "&orderNo=" + orderNo;
	...
	@RequestMapping("bus/toHailInnerCityQrcodeOrderPay")
	...
	@RequestMapping("/hail/innerCity/qrcode/toPaySuccess")
	public String toInnerCityPaySuccess(Model model, String orderNo) {
		...
		return "hail/order/qrcodePaySuccess";
	}
	 ...
	 * @return
	 */
	@RequestMapping("/hail/bus/getInnerCityQrcodeOrderPrepayInfo")
	...
	@RequestMapping("/hail/innerCity/qrcode/toPayFail")

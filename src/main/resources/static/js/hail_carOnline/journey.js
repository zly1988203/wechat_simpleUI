...
$('#priceRule').off('click').on('click', function () {
    window.location = '/hail/onlinecarOrder/billrules?orderNo=' + driverInfo.orderNo + '&type=' + driverInfo.type + '&cityId=' + driverInfo.cityId + '&token=' + $.cookie('token');
});

//拨打电话
$('#calltel').on('click', function () {
    callDriverByServer(SERVER_URL_PREFIX+"/hail",driverInfo.orderNo);
});

$('.commment').on('click', function () {
    if ('1' == $(this).attr('data-type')) {
        window.location.href = '/hail/onlineComment/evaluate?orderNo=' + driverInfo.orderNo + '&token=' + $.cookie('token');
    }
    else if ('2' == $(this).attr('data-type')) {
        window.location.href = '/hail/onlineComment/evaluated?orderNo=' + driverInfo.orderNo + '&token=' + $.cookie('token');
    }
});
    ...
    $.dialog({
        ...
        buttons: [{
            ...
            onClick: function () {
                ...
                $.post("/hail/baseOnlineCar/addComplaint", complaintParams, function (data) {
                    if (data.code == 0) {
...
$('.sos').on('click', function () {
    ...
    $.confirm("是否确定发送紧急求助?", '温馨提示', ['取消', '确定'], function () {
        $.post("/hail/baseOnlineCar/sendSosMsg", params, function (data) {
            if (data.code != 0) {
                //没有设置紧急联系人
                $.confirm(data.message, '温馨提示', ['取消', '去设置'], function () {
                    window.location.href = '/passenger/hail/contacts.html?type=online&orderNo=' + driverInfo.orderNo;
                    //跳转到紧急联系人页面
                }, function () {
...
$('#payBtn').off('click').on('click', function () {
     ...
     var b = new Base64();
     var url = b.encode('/hail/bus/toOnlineCarOrderDetail?orderNo='+driverInfo.orderNo);
     window.location.href='/hail/order/payunit?orderNo='+driverInfo.orderNo+'&settleType='+driverInfo.settleType+'&userCouponId='+userCouponId+'&url='+url;

//    var orderNo = $('#orderNo').val();
//    window.location = '/hail/bus/toOnlineCarOrderDetail?token=' + $.cookie('token') + '&orderNo=' + orderNo + '&toPay=1&recordId=' + $('#recordId').val();
});
...
$('#couponsBox').off('click').on('click', function () {
    ...
    if(clickable){
        window.location = '/coupon/select?orderNo='+driverInfo.orderNo+'&businessType='+businessParam.innerCityOnline+'&cityId='+driverInfo.cityId+'&price='+$('#oldPrice').val()+"&specialFlag=0";
    }else{
        return;
    }
})
...
function  shareUserLine() {
    var reqUrl = CURRENT_SERVER + '/hail/baseOnlineCar/share?orderNo=' + driverInfo.orderNo + '&driverLng=111.22&driverLat=1111.222';
    var shareObj = {
    ...
    var wss = new communicate({
        ...
        messageCallback: function(event) {
            ...
            if((temptType == 4 || temptType==8 || temptType==9) && serviceId == '1006'){
                ...
                if (data.orderNo) {
                    //立即支付
                    window.location = '/hail/bus/toOnlineCarOrderDetail?orderNo=' + driverInfo.orderNo + '&token=' + $.cookie('token') + '&toPay=1&recordId=' + $('#recordId').val();
                }
            }
        }
    })

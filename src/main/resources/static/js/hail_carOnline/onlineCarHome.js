...
var checkCityIsOpen = function(lng,lat,type){
    ...
    var url = '/hail/baseOnlineCar/checkCityIsOpen';
...
$(function() {
    ...
    //初始化加载业务当行蓝
    getBusinessTypes("innerCityOnline");
...
function getAgreement(providerId) {
    ...
    var url = '/hail/baseOnlineCar/providerAgreement';
    $.post(url,params,function(data){
        var content = '暂无协议';
        if(data && data.code == 0){
...
function showBillingLoading() {
    ...
    $.post("/hail/onlineTrip/tripPrice",params,function(data){
...
var submitTrip = function(){
    ...
    $.post("/hail/onlineTrip/addTrip",params,function(data){
...
var checkUnfinishTripAndOrder = function(){
    var url = '/hail/onlineTrip/checkUnfinishedTripAndOrder';
    var params = {
        token : $.cookie('token')
    }
...
var cancelTrip = function(tripNo,type){
    ...
    var params = {
        tripNo : tripNo,
        token : $.cookie('token'),
        cancleType:cancleTypeTemp
    }
    var postUrl = '/hail/onlineTrip/cancelTrip';
...
var getPriceRule = function(cityId,departType){
    ...
    var postUrl = '/hail/baseOnlineCar/priceRule';
    $.post(postUrl,params,function(data){
        $.hideLoading();
        if(data.code == 0){
...
var getCarNumbers = function(){
    ...
    $.post('/hail/baseOnlineCar/carNumber',params,function(data){
        var carNumbers = 15;
        if(data && data.code == 0){
            if(data.configValue){
...
function retryInitTrip(tripNo){
    ...
    $.post('/hail/onlineTrip/retryInitTrip', param, function(result){});
}
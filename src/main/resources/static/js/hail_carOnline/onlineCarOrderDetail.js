...
$(function() {
    backtoUrl('/passenger/innerCityOnlineOrderList.html');
    var params = {
        'orderNo' : orderNo,
        'token' : $.cookie('token')
    }
    $.post("/hail/onlinecarOrder/queryOrderdetail",params,function(data){
        ...
        cityId = detailData.departCityId;//出发地城市id
        priceTemp = detailData.price;
        ...
        var   station_html =  '<div class="time">'+timeStr+'</div>' +
            ...
            '<div class="oldprice"><div>一口价</div>'+
            '<div class="order-price">'+detailData.price+'元</div> </div>'+
        ...
        var _html = '';
        if(order_status == 0){
            _html += '<div class="pay-wait">待支付&emsp;<label>'+detailData.price+'</label>元</div>' +
            ...
            if(parseFloat(detailData.couponMoney) > parseFloat(detailData.price)){
                couponMoney = parseFloat(detailData.price);
            }
            _html += '<div class="time">'+payTime+'</div>' +
                '<div class="price">已支付&emsp;'+detailData.payPrice+'元</div>' +
                '<div class="oldprice2">' +
                '<div style="width: 50%">一口价</div>'+
                '<div style="width: 50%">'+detailData.price+'元</div> </div>'+
...
//            payConfirm(providerId,sellProviderId);
            ...
            var b = new Base64();
            var url = b.encode('/hail/bus/toOnlineCarOrderDetail?orderNo='+orderNo);
            window.location.href='/hail/order/payunit?orderNo='+orderNo+'&settleType='+settleType+'&userCouponId='+userCouponId+'&url='+url;
        });
        $('.evaluate-btn.evaluate-wait').off('click').on('click',function(){
            window.location = '/hail/onlineComment/evaluate?orderNo=' + orderNo + "&token=" + $.cookie('token');
        });
        $('.evaluate-btn.evaluated').off('click').on('click',function(){
            window.location = '/hail/onlineComment/evaluated?orderNo=' + orderNo + "&token=" + $.cookie('token');
        });
    },"json");
    ...
    var couponClickEvent = function () {
        $('#amountTxt').off('click').on('click',function(){
            if($(this).data('clickable')){
                window.location = '/coupon/select?orderNo='+$('#orderNo').val()+'&businessType='+businessParam.innerCityOnline+'&cityId='+cityId+'&price='+$('#oldPrice').val()+"&specialFlag=0";
            }else{
                return;
            }
        })

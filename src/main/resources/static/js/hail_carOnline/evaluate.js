$(function() {
    ...
    $('.star').each(function (index) {
        ...
        $el.children('span').on('click', function () {
            ...
            $.post("/hail/onlineComment/queryCommentTag",params,function(data){
                if(data.code!='0'){//异常
                    $.alert(data.msg)
                }
    ...
    $("#confirmData").off('click').on('click', function () {
        ...
        $.post("/hail/onlineComment/submitComment",params,function(data){
            if(data.code!='0'){
                $.alert(data.msg);
                return;
            }
            //成功后跳转到已评价页面
            window.location.href = "/hail/onlineComment/evaluated?orderNo="+orderNo+"&token="+$.cookie('token');
        },"json");
    });
});

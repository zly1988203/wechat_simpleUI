...
$(function () {
    ...
    //header初始化
    getBusinessTypes("interCityOnline");//初始化header导航,引用js/commonjs/header.js
...
//联系客服
/*$('#contact').off(clickEvent).on(clickEvent, function() {
    $('#sideMenu').closePopup(function() {
        var urlDetail = SERVER_URL_PREFIX + '/hail/Config/getBusinessTel';
        var dataDetail = {
        };
...
function loadHistoryRcord(){
    var url = SERVER_URL_PREFIX + '/hail/innerCity/optimize/recommendTrip';
    var dataObj = {
    };
    dataObj = genReqData(url, dataObj);
    dataObj['token']=$.cookie('token');
...
function loadAlltrip() {
    var url = SERVER_URL_PREFIX + '/hail/innerCity/optimize/queryNewestOrder';
    var dataObj = {
    };
    dataObj = genReqData(url, dataObj);
    dataObj['token']=$.cookie('token');

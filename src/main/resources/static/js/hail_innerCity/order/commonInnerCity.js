...
function  innerCityUrlJudge(tripNo,callBack,orderNo){
	$.ajax({
		url:'/hail/innerCity/order/getInnerCityTripStatus',
...
function loadCoupons() {
    ...
    //目前城际网约车是不使用优惠券，所以现在直接赋值
//    $('#newPayPrice').val(price);
...
$('#payPanel #coupon').on(clickEvent,function () {
    ...
    window.location = '/coupon/select?orderNo='+$('#orderNo').val()+'&businessType='+businessParam.interCityOnline+'&lineId='+$('.journey-content').data('lineid')+"&specialFlag="+specialFlag+"&price="+$('#price').val();
});
...
$('#paymentBtn').on(clickEvent,function(){
    ...
    window.location ='/hail/order/payunit?orderNo='+orderInfo.orderNo+'&settleType='+orderInfo.settleMode+'&userCouponId='+userCouponId+'&url='+url;
});
    ...
    $('#cancel').on('click',function(){
        $.confirm('确定取消订单吗？', '',['暂不取消', '确定取消'],function() {
            if (orderInfo.status == 4 || orderInfo.status == 5){
                ...
                $(".payment-info").show();
                //取消并退款
                location.href='/hail/innerCity/order/toOrderCancelReturnPage?orderNo='+orderInfo.orderNo;
            }

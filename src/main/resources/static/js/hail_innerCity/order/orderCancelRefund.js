...
$("#refund").on("click",function(){
    ...
    $.dialog({
        ...
        buttons: [{
            ...
            text: "确定",
            onClick: function() {
                $.post("/hail/innerCity/order/refund",{orderNo:orderNo,message:$("#message-1").val()},function(result){
                    if(result.code==0){
                        $.alert(result.message);
                        location.href='/hail/innerCity/order/toOrderReturnDetailPage?orderNo='+orderNo;
                    }else{
                        $.dialog({
                            title: "提示",
                            text: result.message,
...
$('#notRefund').on(clickEvent,function(){
    location.href="/hail/innerCity/order/toOrderDetail?orderNo=" + $('#orderNo').val();
});

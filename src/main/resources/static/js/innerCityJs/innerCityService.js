        ...
        success: function(data){
            if(data.code ==0){
                ...
                $.confirm('您当前有未完成的行程', '',[ '我知道了','进入行程'], function() {
                    ...
                    // window.location.href="/innerCity/order/toOrderDetail?orderNo="+data.data.orderNo;
                });
            }
        }
    });
            ...
            $.ajaxService({
                ...
                success: function(data){
                    ...
                    if(data.code ==0){
                        ...
                        window.location.href='/innerCity/order/toOrderPay?orderNo='+data.data.orderNo+'&type=1&v='+timestamp;
                    }else{
                        $.alert(data.message);
                    }
                }
...
$('.btn-bar .tips-btn').on(click_tap,function () {
    var urlDetail ='/innerCity/reminder';
    var dataDetail = {};
...
$('.tips-container-popup .service-rules-btn').on(click_tap,function () {
    window.location.href = '/innerCity/order/serviceRules';
});
...
function judgeService(param) {
    $.showLoading();
    var url = SERVER_URL_PREFIX+'/innerCity/optimize/judgeService';
    var dataObj = param;
    dataObj.alone = true;
    ...
    $.ajaxService({
        ...
        success:function (res) {
            ...
            if(null != res && res.code == 0){
                ...
                clearStartTime();
                if($('.sui-dialog')==null||$('.sui-dialog').length<=0){

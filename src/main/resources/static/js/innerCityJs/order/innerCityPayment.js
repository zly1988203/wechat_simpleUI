...
// 查看优惠券
$('#has-coupon dd').off(clickEvent).on(clickEvent,function(){
    window.location = '/coupon/select?orderNo='+orderInfo.orderNo+'&businessType='+businessParam.innerCity;
});
//取消订单
...
$("#orderPay").on(clickEvent,function(){//获取预支付信息
    ...
    window.location.href='/order/payunit?orderNo='+orderInfo.orderNo+'&settleType='+orderInfo.settleMode+'&userCouponId='+userCouponId+'&url='+url;
});

// 点击稍后支付
$('#laterPay').off(clickEvent).on(clickEvent,function () {
    window.location='/innerCity/orderDetail/getOrderInfo?orderNo='+orderInfo.orderNo;
});

// 返回
$(".return").off(clickEvent).on(clickEvent,function () {
    //清空缓存

...
//        analyse([116.39392,39.913828]);
        ...
        if('${departDate!''}'!=''){
    		...
    		if(year == 'NaN' || month == 'NaN' || day == 'NaN' || week == 'undefined'){
    			...
    			$('#startTime').val(str);
    			$('#startTime').data('date','${departDate!''}');
 ...
 */                setTimeout(function() {
        ...
        $(".hotline").click(function() {
        	...
        	+ "&&lng=" + lineParam.departLng + "&&lat=" + lineParam.departLat+"&lineName="+lineName + "&lineType=" + ${travelLineType!'3'};
        });
        $(".historyLine").click(function(){
        	...
        	$.post(url,data,function(result){
        		if(result.code == 0){
        			...
        			if(baseBusCount > 0){
        				window.location="/bus/lineList?busId=" + busId + "&&token="+$.cookie("token") + "&lineType=" + ${travelLineType!'3'};
        			}else{
        				$.toast('当前无可售班次');
        				return false;
        			}
    ...
    var getCityAndArea = function(){
    	...
    	$.post(url,{lineType:${travelLineType!'3'}},function(data){
    		var code = data.code;
    		if(code != 0){
    			return false;
    		} 

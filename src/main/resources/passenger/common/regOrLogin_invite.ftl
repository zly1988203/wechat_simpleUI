...
<head>
    ...
    <link href="/res/style/register.css" rel="stylesheet" type="text/css">
</head>

<body>
	<#include "../foot.ftl"/>
	<div class="logo"></div>
	<div class="tips">为了司机方便联系您，请验证手机号码</div>
	...
	<div class="form">
	    ...
	    <button id="submitButton" type="button" class="submit-btn">确定</button>
	</div>
             ...
             $.post(urlStr, dataObj, function(result){
                    ...
                    if(result && result.code == 0){
                    	if(result.data.token){
                    		...
                    		//window.location='https://open.weixin.qq.com/connect/oauth2/authorize?appid='+result.data.appID+'&redirect_uri=' + CURRENT_SERVER + '/index&connect_redirect=1&response_type=code&scope=snsapi_base&state='+(result.data.token+"_"+result.data.providerId)+'#wechat_redirect';
                            window.location = getWechatAuthUrl(result.data.appID, CURRENT_SERVER + '/index', (result.data.token+"_"+result.data.providerId));
                    	}
                    }else{
